...
class PostgresBackend(xa: TransactorZIO) extends Repo[DashboardCreator, DashboardEntity, Long] with Backend:
...
object PostgresBackend:
  ...
  /** Creates the database schema for dashboards. This should be called once during application initialization.
    */
  ...
  /** Creates a PostgresBackend instance with the given Transactor. The schema must be created separately using createSchema.
    */
  def make(xa: TransactorZIO): UIO[PostgresBackend] =
    ZIO.succeed(new PostgresBackend(xa))

  /** Creates a ZLayer that provides a PostgresBackend. Requires a Transactor to be provided.
    */
  val layer: URLayer[TransactorZIO, PostgresBackend] =
    ZLayer.fromFunction((xa: TransactorZIO) => new PostgresBackend(xa))

  /** Creates a Transactor from a DataSource. This is a convenience method for setting up the database connection.
    */
  def transactorLayer(dataSource: DataSource): ULayer[Transactor] =
    ZLayer.succeed(Transactor(dataSource))

  /** Creates a complete ZLayer that provides both schema initialization and PostgresBackend. This will create the schema if it doesn't
    * exist and then provide the backend.
    */
