...
/** PostgreSQL-specific configuration for database connections.
  *
  * @param jdbcUrl
  *   JDBC URL for PostgreSQL (e.g., "jdbc:postgresql://localhost:5432/mydb")
  * @param username
  *   Database username
  * @param password
  *   Database password
  ...
  * @param maxLifetime
  *   Maximum lifetime of a connection in milliseconds (default: 1800000)
  */
...
/** PostgreSQL implementation of the DataSource trait.
  *
  * Provides PostgreSQL-specific functionality for executing SQL queries and returning results as JSON. Uses HikariCP for connection pooling
...
class PostgresDataSource(
  ...
  /** Executes the SQL query from the DataBinding and returns results as JSON.
    *
    * The SQL query can contain parameter placeholders in the format {{paramName}}. These will be replaced with values from the params map.
    *
    * @param dataBinding
    *   Contains the SQL query to execute
    * @param params
    *   Map of parameter names to JSON values for query substitution
    * @return
    *   Task containing JSON array of result rows
    */
  ...
  /** // FIXME neist po parametroch ale po vsetkych vyskytoch premenntych v slq Substitutes parameter placeholders in SQL with actual
    * values. Placeholders are in the format {{paramName}}.
    */
  ...
  /** Executes a SQL query and converts the result set to JSON. This method runs within a Magnum transaction context.
    */
  ...
  /** Converts a JDBC ResultSet to a JSON array of objects. Each row becomes a JSON object with column names as keys.
    */
  ...
  /** Extracts a column value from the ResultSet and converts it to JSON. Handles PostgreSQL-specific types appropriately.
    */
  private def getColumnAsJson(rs: ResultSet, metaData: ResultSetMetaData, columnIndex: Int): Json =
    if rs.getObject(columnIndex) == null then Json.Null
    else
  ...
  /** Closes the underlying HikariCP connection pool. Should be called when the data source is no longer needed.
    */
  def close(): Task[Unit] =
...
object PostgresDataSource:
  ...
  /** Creates a PostgresDataSource from configuration. The returned resource is scoped and will automatically close the connection pool.
    */
  ...
  /** Creates a ZLayer that provides a PostgresDataSource. Requires PostgresConfig to be provided.
    */
